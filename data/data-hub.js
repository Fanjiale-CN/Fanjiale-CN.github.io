  ...
  class DataHub {
    ...
    render() {
      ...
      this.tableWrap.innerHTML = `
        <table>
          <thead><tr><th>年份</th><th>${ind.name} (${ind.unit})</th></tr></thead>
          <tbody>
            ${entries.map((e) => `
        ...
        <p class="data-hub-source">来源：<a href="${ind.sourceUrl}" target="_blank" rel="noopener">${ind.source}</a></p>
      `;
    }
    ...
    showTooltip(entry, x, y) {
      ...
      this.tooltip.innerHTML = `<b>${entry.year}</b><span>${entry.value.toFixed(1)}%</span>${pandemic ? '<em>疫情三年</em>' : ""}`;
    }
