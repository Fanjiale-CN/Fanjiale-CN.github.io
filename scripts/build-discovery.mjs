...
const excludedDirectories = new Set([".git", "node_modules", "pagefind", "_research-source", "archive", "qa", "video", "artifacts", "hyperframes"]);
...
function metaContent(html, name) {
  ...
  const expression = new RegExp(`<meta[^>]+(?:name|property)=["']${escaped}["'][^>]+content=["']([^"']*)["'][^>]*>`, "i");
...
function releaseDate() {
  ...
  const parts = new Intl.DateTimeFormat("en-US", { timeZone: releaseTimezone, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(new Date());
